...
ISTA作为宝马维修车间系统，供全世界经销商机构中使用的BMW Group车辆诊断和编程应用使用。作为经销商使用的软件，宝马为该程序添加了许可证校验无可厚非。但是它在互联网流传过程中，不但其本体获取存在一定的困难，其许可证的获取也颇有意思，与我们通常能够看到的破解软件有着一定的区别。近日在网上获取到一套完整的安装程序，故对其进行了对应的探究。
...
对于完整性校验`IntegrityManager`，共涉及3个`exe`，具体如下
- ISTAGUI.exe
- IstaOperation.exe
- IstaServicesHost.exe

对于完整性校验`CheckSignature`，共涉及1个`dll`，具体如下
- RheingoldCoreFramework.dll

对于激活码校验`IsLicenseValid`，共涉及1个`exe`、21个`dll`，具体如下
- Authoring.dll
