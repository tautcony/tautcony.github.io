...
| `assets-jekyll.json` | 需继续发布的静态资源 URL、字节数、sha256；不含已退役资源 | M3+ |
...
资源 manifest 使用稳定排序和 sha256；所有 fixture 记录 source commit、工具版本和生成命令。新增或退役路径必须显式更新 manifest；当前已退役 r56 专用的 `/img/404/{disc.png,inner_bck.jpg,particle_tr.png}`。
