...
以上就是系统能够正常查看维护手册所需要进行的补丁内容，由于没有使用过正常激活的程序，有一些可能还是有所遗漏。
...
但是在使用过程中，日志里还是能看到许多提示激活失败的记录，使用却无大碍，经排查，可以在注册表或者配置文件增加一个证书，确保程序在一些情况下，能正常读取到一个非空的证书信息，以减少这类报错。
...
应用完整性校验
- `IntegrityManager::.ctor`
- `IstaProcessStarter::CheckSignature`
- `VerifyAssemblyHelper::VerifyStrongName`

证书有效性校验
- `LicenseStatusChecker::IsLicenseValid`
- `LicenseManager::VerifyLicense`
- `LicenseAOSManager::VerifyLicense`
- `IstaIcsServiceClient::VerifyLicense`
- `CommonServiceWrapper::VerifyLicense`
- `LicenseWizardHelper::DoLicenseCheck`
- `CertificateHelper::ValidateCertificate`

其它校验
- `IstaIcsServiceClient::ValidateHost`
- `SecureAccessHelper::IsCodeAccessPermitted`
- `FscValidationClient::IsValid`
- `MainWindowViewModel::CheckExpirationDate`
- `ActivationCertificateHelper::IsInWhiteList`
- `ActivationCertificateHelper::IsWhiteListSignatureValid`
- `ConfigurationService::SetPsdzProperties`
