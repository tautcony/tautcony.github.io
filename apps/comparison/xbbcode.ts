module XBBCODE {
    ...
    interface ITag {
        ...
        restrictParentsTo?: Array<string>;
        restrictChildrenTo?: Array<string>;
        noParse?: boolean;
    ...
    let tagsNoParseList: Array<string> = [];
    ...
    function initTags() {
        ...
        for (let prop in tags) {
            if (tags.hasOwnProperty(prop)) {
                ...
                let len = tags[prop].restrictChildrenTo.length;
                for (let i = 0; i < len; ++i) {
    ...
    // private functions
    // -----------------------------------------------------------------------------
    function checkParentChildRestrictions(parentTag: string, bbcode: string, bbcodeLevel: number, tagName: string, tagParams: string, tagContents: string, errQueue?: Array<string>): Array<string> {
        ...
        let reTagNamesParts = new RegExp("(<bbcl=" + bbcodeLevel + " )(" + tagList.join("|") + ")([ =>])", "i");
        let matchingTags = tagContents.match(reTagNames) || [];

        let pInfo: ITag = tags[parentTag] || undefined;
        ...
        for (let matchingTag of matchingTags) {
            ...
            let childTag = (matchingTag.match(reTagNamesParts))[2].toLowerCase();

            if (pInfo && pInfo.restrictChildrenTo && pInfo.restrictChildrenTo.length > 0) {
                if (!pInfo.validChildLookup[childTag]) {
                    let errStr = "The tag \"" + childTag + "\" is not allowed as a child of the tag \"" + parentTag + "\".";
                    errQueue.push(errStr);
                }
            }
            let cInfo: ITag = tags[childTag] || undefined;
            if (cInfo.restrictParentsTo.length > 0) {
                if (!cInfo.validParentLookup[parentTag]) {
                    let errStr = "The tag \"" + parentTag + "\" is not allowed as a parent of the tag \"" + childTag + "\".";
                    errQueue.push(errStr);
                }
            }
        }
    ...
    */
    ...
    export function addTags(newtags: Array<ITag>): void {
    ...
    export function process(config: IConfig): IResult {
        let ret: IResult = { html: "", error: false, errorQueue: [] };
