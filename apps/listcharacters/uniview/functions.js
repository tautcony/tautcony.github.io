...
function addLine(codepoint, newContent) {
    ...
    var uplus = '';
    if (document.getElementById('uPlusToggle').checked) {
        uplus = 'U+';
    }
...
function changeFont(fontName) {
    ...
    if (isMatrix()) {
        ...
        nodeArray = leftpanel.getElementsByTagName('span');
        for (var i = 0; i < nodeArray.length; i++) {
...
function doN11n(form) {
    ...
    if (form == 'nfc') {
        document.getElementById('picker').value = nfc(str);
    } else {
        document.getElementById('picker').value = nfd(str);
    }
...
function convert2upper(string, detail) {
    ...
    if (detail) {
        for (var i = 0; i < codepoints.length; i++) {
            ...
            if (notfound) {
                singletons += ' ' + getCharFromInt(parseInt(cRecord[0], 16));
            }
        }
    } else {
        for (var i = 0; i < codepoints.length; i++) {
            ...
            if (cRecord[12]) {
                ...
                uppercase += getCharFromInt(parseInt(cRecord[0], 16));
            }
            if (cRecord[13]) {
                lowercase += getCharFromInt(parseInt(cRecord[13], 16));
                notfound = false;
            } else {
                lowercase += getCharFromInt(parseInt(cRecord[0], 16));
            }
            if (cRecord[14]) {
                titlecase += getCharFromInt(parseInt(cRecord[14], 16));
                notfound = false;
            } else {
                titlecase += getCharFromInt(parseInt(cRecord[0], 16));
            }
            if (notfound) {
                singletons += ' ' + getCharFromInt(parseInt(cRecord[0], 16));
            }
            ...
            if (divs[i].className.match(/ch/)) {
                ...
                if (document.getElementById('graphicsToggle').checked == true) { // change span to img
                    ...
                    if (cRecord[GRAPHIC_FIELD] == 'm') {
                        ...
                        //scriptGroup = findScriptGroup(parseInt(titlefields[3]));
                        //if (localGraphics.indexOf(scriptGroup+',') > -1) {
                        //img.src = 'graphics/'+scriptGroup+'/'+hexNum+'.png'; } else { img.src = 'http://decodeunicode.org/data/glyph/26x26/' + hexNum + '.gif'; dugraphics = true; }
...
function foundInList(ch, range) {
    ...
    for (i = 0; i < runs.length; i++) {
        ...
        } else if (startEnd.length > 1 && (ch >= parseInt(startEnd[0]) && ch <= parseInt(startEnd[1]))) {
            return true;
        }
    }
    return false;
...
function charInfoPointer(codepoint) {
    ...
    if (i == scriptGroups.length) { return ''; }
        ...
        if (foundInList(charNum, scriptGroups[i][4])) {
            return (scriptGroups[i][3]);
        } else {
            return '';
        }
    }
